...
### Phase 85: Search Quality Observation Release Gate ✅ COMPLETED

- [x] stale local observation과 target provenance 부재가 release closeout에서 탐지되지 않는 evidence gap 재현
- [x] fresh diagnostics snapshot, deployment provenance, workspace fingerprint를 결합한 fail-close contract 추가
- [x] search-quality report schema와 release closeout report에 observation evidence 상태 연결
- [x] README/roadmap/evidence docs에 observation gate와 directional-only 해석 경계 반영
- [x] targeted contract, adapter/domain suite, typecheck, build, local observation/release evidence 재검증

### Phase 84: Product Grouping Quality Benchmark ✅ COMPLETED

- [x] labeled cross-mall fixture와 pairwise precision/recall/F1 평가 contract 추가
- [x] `P-6000`/`P6000` model-code false split을 letter-to-digit hyphen normalization으로 수정
- [x] production `groupProducts()` 기준 benchmark verifier와 fingerprint-linked artifact/release report gate 추가
- [x] README/roadmap/evidence docs에 측정 정의, 실행 명령, curated fixture 한계 반영
- [x] targeted contract, adapter/domain suite, typecheck, build, release evidence 재검증
...
### Phase 82: Release QA Artifact Persistence ✅ COMPLETED

- [x] `ntl:release-qa-smoke` 성공 JSON이 stdout에만 남아 stable artifact가 갱신되지 않는 결함 재현
- [x] local/deployed target별 stable output path를 선택하고 `RELEASE_QA_OUTPUT_PATH` override 지원
- [x] `tee`로 기존 JSON stdout contract를 유지하면서 artifact를 자동 저장하도록 수정
...
- [x] Compare Entry handoff 문서 현재 상태 동기화 완료 (2026-06-18): README와 `docs/COMPARE_ENTRY_REVIEW_GATE.md` 상단을 `SUN-10 READY` / `SUN-11~13 완료` / active blocker `none` 기준으로 갱신하고, historical Figma quota unblock 절차를 운영 매뉴얼 하위 경로로 정리. `npm run ntl:compare-entry-review-ready-check` 재검증 결과 gate `READY`, artifact audit `READY`, active blocker `none` 으로 통과
...
- [x] SUN-12 구현 완료 (2026-06-12): 검색결과 비교 계층 — 메트릭 카드 액센트 바, HighlightCard 배지 클러스터(Compare Ready 액센트 필 선두), ResultCard 신뢰 배지 상시 노출 + 몰 비교 배지 잉크/옐로, CompareShortlistActionButton '+ 비교 후보 담기' 액센트화, shortlist 섹션을 다크 연속성 존으로 통일. 부수 발견: useCompareShortlist의 setState-updater 내 side effect 버그(렌더 중 타 컴포넌트 setState) 별도 커밋으로 수정. grouping/모달 진입/favorite payload 보존, typecheck·테스트 245·로컬 시각/기능 검증 통과
- [x] SUN-13 검증 완료 (2026-06-12): SUN-11/12 프로덕션 배포 후 browser-smoke(13/22건)·compare-entry-baseline 갱신·release-qa-smoke·favorites-probe(firestore write/visible/click-through/compare reachable/cleanup 전부 verified)·auth-release-qa(synthetic coverage 4/4) 통과. favorites 프로브의 15s 클라이언트 타임아웃이 서버 12s 집계 예산+콜드스타트와 충돌하던 만성 플래키를 35s로 완화
- [x] 릴리즈 사후 코드 리뷰 (2026-06-12): 오늘 머지된 7커밋(체인 제네릭화·admin UI 통합·SUN-11/12 리디자인)을 영역별 adversarial 3-에이전트 리뷰. 기능 회귀 0건(잘못된 Firestore context·동작 손실 없음) 확인. 수정: sibling-nav note를 aria-label로 복원(title= hover-only 정보손실), 다크 셸 라벨 slate-500→400(WCAG AA), playbook root >=3 의도 주석, useOpsChainActions의 dead missingFallback 필드를 단일 queue 핸들러로 wire. 가시성 정규화(완료 레인 G2/G4 토글 뒤)는 사용자 결정으로 현 상태 유지
