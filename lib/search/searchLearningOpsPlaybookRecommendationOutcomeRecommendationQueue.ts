import type {
    ...
    SearchLearningOpsPlaybookRecommendationOutcomeRecommendationAction,
    SearchLearningOpsPlaybookRecommendationOutcomeRecommendationSummary,
} from './searchLearningOpsPlaybookRecommendationOutcomeRecommendations.ts';

export type SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueState =
    | 'execute_now'
    | 'needs_review'
    | 'sample_collection'
    | 'observe';

export type SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem = {
    id: string;
    recommendationId: string;
    title: string;
    description: string;
    reason: string;
    queueState: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueState;
    action: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationAction;
    actionLabel: string;
    priority: 'critical' | 'high' | 'medium' | 'low';
    outcomeStatus: SearchLearningOpsPlaybookRecommendationOutcomeRecommendation['outcomeStatus'];
    createdAt: string;
    entryIds: string[];
    queries: string[];
...
export type SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueSummary = {
    total: number;
    executeNow: number;
    needsReview: number;
    ...
    observe: number;
    urgent: number;
    topExecuteNow: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[];
    topNeedsReview: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[];
    topSampleCollection: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[];
    topObserve: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[];
};

function resolveQueueState(
    action: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationAction
): SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueState {
    switch (action) {
        case 'retrain_now':
            return 'execute_now';
        case 'review_now':
...
function stateWeight(state: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueState): number {
    switch (state) {
        case 'execute_now':
            return 4;
        case 'needs_review':
...
): SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem {
    return {
        id: `playbook_recommendation_outcome_recommendation_queue:${recommendation.id}`,
...
function sortQueue(
    items: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[]
): SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueItem[] {
...
export function buildSearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueue(
    summary: SearchLearningOpsPlaybookRecommendationOutcomeRecommendationSummary
): SearchLearningOpsPlaybookRecommendationOutcomeRecommendationQueueSummary {
