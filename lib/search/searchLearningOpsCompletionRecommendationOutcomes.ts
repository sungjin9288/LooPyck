...
export type SearchLearningOpsCompletionRecommendationOutcomeEntryLike = {
    id: string;
    query: string;
...
export type SearchLearningOpsCompletionRecommendationOutcome = {
    id: string;
    activityId: string;
    title: string;
    ...
    queries: string[];
    entryIds: string[];
    status: SearchLearningOpsCompletionRecommendationOutcomeStatus;
    description: string;
...
export type SearchLearningOpsCompletionRecommendationOutcomeSummary = {
    total: number;
    readyReview: number;
    needsAttention: number;
    awaitingSamples: number;
    validated: number;
    topReadyReview: SearchLearningOpsCompletionRecommendationOutcome[];
    topNeedsAttention: SearchLearningOpsCompletionRecommendationOutcome[];
    topAwaitingSamples: SearchLearningOpsCompletionRecommendationOutcome[];
    topValidated: SearchLearningOpsCompletionRecommendationOutcome[];
};
...
function sortOutcomes(
    items: SearchLearningOpsCompletionRecommendationOutcome[]
...
export function buildSearchLearningOpsCompletionRecommendationOutcomes(
    ...
    const outcomes: SearchLearningOpsCompletionRecommendationOutcome[] = [];

    for (const run of runs) {
        const relatedEntries = run.entryIds
            .map((entryId) => entryMap.get(entryId))
            .filter((entry): entry is SearchLearningOpsCompletionRecommendationOutcomeEntryLike => Boolean(entry));
        ...
        let status: SearchLearningOpsCompletionRecommendationOutcomeStatus = 'validated';
        let description = 'completion recommendation 실행이 승인 상태와 검색 개선 지표까지 안정적으로 이어졌습니다.';

        if (readyReviewCount > 0) {
            status = 'ready_review';
            description = 'completion recommendation 실행으로 새 AI suggestion이 준비되어 draft review/승인이 필요한 상태입니다.';
        } else if (noImprovementCount > 0) {
            status = 'needs_attention';
            description = 'completion recommendation 실행 후에도 low-fit/0건 개선이 약해 재학습 또는 rewrite 조정이 필요합니다.';
        } else if (awaitingSamplesCount > 0 || improvedCount === 0) {
            status = 'awaiting_samples';
            description = 'completion recommendation은 실행됐지만 아직 표본이 부족해 실제 개선 여부를 더 지켜봐야 합니다.';
        }

        outcomes.push({
            id: `completion_recommendation_outcome:${run.id}`,
            activityId: run.id,
            title: run.title,
            action: run.action,
            context: run.context,
