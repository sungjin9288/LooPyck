import type {
    SearchLearningOpsCompletionOutcome,
    SearchLearningOpsCompletionOutcomeStatus,
    SearchLearningOpsCompletionOutcomeSummary,
} from './searchLearningOpsCompletionOutcomes.ts';

export type SearchLearningOpsCompletionRecommendationAction =
    | 'review_now'
    | 'retrain_now'
    | 'collect_samples'
    | 'observe';

export type SearchLearningOpsCompletionRecommendation = {
    id: string;
    outcomeId: string;
    title: string;
    description: string;
    ...
    outcomeStatus: SearchLearningOpsCompletionOutcomeStatus;
    createdAt: string;
    entryIds: string[];
    queries: string[];
...
export type SearchLearningOpsCompletionRecommendationSummary = {
    total: number;
    reviewNow: number;
    retrainNow: number;
    ...
    critical: number;
    highPriority: number;
    topReviewNow: SearchLearningOpsCompletionRecommendation[];
    topRetrainNow: SearchLearningOpsCompletionRecommendation[];
    topCollectSamples: SearchLearningOpsCompletionRecommendation[];
    topObserve: SearchLearningOpsCompletionRecommendation[];
};

function classifyOutcome(
    outcome: SearchLearningOpsCompletionOutcome
): Pick<
    ...
    switch (outcome.status) {
        case 'ready_review':
            return {
                ...
                reason: `${outcome.readyReviewCount}개의 review 대기 query가 completion outcome에서 준비됐습니다.`,
            };
...
function sortRecommendations(
    items: SearchLearningOpsCompletionRecommendation[]
): SearchLearningOpsCompletionRecommendation[] {
...
function toRecommendation(
    ...
    return {
        id: `completion_recommendation:${outcome.id}`,
...
export function buildSearchLearningOpsCompletionRecommendations(
    outcomes: SearchLearningOpsCompletionOutcomeSummary
