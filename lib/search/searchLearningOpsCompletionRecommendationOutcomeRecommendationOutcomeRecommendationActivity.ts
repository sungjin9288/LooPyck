...
export type SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationActivitySummary = {
    totalRuns: number;
    reviewRuns: number;
    retrainRuns: number;
    uniqueQueries: number;
    recentRuns: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun[];
};
...
function parseRecommendationContext(
    context: string | null
): { outcomeId: string; action: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationAction } | null {
    if (!context) {
        return null;
    }

    if (context.startsWith('completion_recommendation_outcome_recommendation_outcome_review_')) {
        return {
            outcomeId: context.slice('completion_recommendation_outcome_recommendation_outcome_review_'.length),
            action: 'review_now',
        };
    }

    if (context.startsWith('completion_recommendation_outcome_recommendation_outcome_retrain_')) {
        return {
            outcomeId: context.slice('completion_recommendation_outcome_recommendation_outcome_retrain_'.length),
            action: 'retrain_now',
        };
    }
...
function resolveRunMeta(
    ...
    SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun,
...
function toRun(event: SearchLearningActivityEvent): SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun | null {
...
export function buildSearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationActivity(
    events: SearchLearningActivityEvent[]
): SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationActivitySummary {
    const runs = events
        .map(toRun)
        .filter((run): run is SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun => Boolean(run));
