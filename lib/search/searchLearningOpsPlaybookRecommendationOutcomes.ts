...
import type { SearchLearningOpsPlaybookRecommendationRun } from './searchLearningOpsPlaybookRecommendationActivity.ts';

export type SearchLearningOpsPlaybookRecommendationOutcomeStatus =
    | 'ready_review'
    | 'needs_attention'
    | 'awaiting_samples'
    | 'validated';

export type SearchLearningOpsPlaybookRecommendationOutcomeEntryLike = {
    id: string;
    query: string;
...
export type SearchLearningOpsPlaybookRecommendationOutcome = {
    id: string;
    outcomeId: string;
    title: string;
    action: SearchLearningOpsPlaybookRecommendationRun['action'];
    context: string;
    createdAt: string;
    queries: string[];
    entryIds: string[];
    status: SearchLearningOpsPlaybookRecommendationOutcomeStatus;
    description: string;
...
export type SearchLearningOpsPlaybookRecommendationOutcomeSummary = {
    total: number;
    readyReview: number;
    needsAttention: number;
    awaitingSamples: number;
    validated: number;
    topReadyReview: SearchLearningOpsPlaybookRecommendationOutcome[];
    topNeedsAttention: SearchLearningOpsPlaybookRecommendationOutcome[];
    topAwaitingSamples: SearchLearningOpsPlaybookRecommendationOutcome[];
    topValidated: SearchLearningOpsPlaybookRecommendationOutcome[];
};
...
function sortOutcomes(
    items: SearchLearningOpsPlaybookRecommendationOutcome[]
...
export function buildSearchLearningOpsPlaybookRecommendationOutcomes(
    runs: SearchLearningOpsPlaybookRecommendationRun[],
    entries: SearchLearningOpsPlaybookRecommendationOutcomeEntryLike[]
): SearchLearningOpsPlaybookRecommendationOutcomeSummary {
    const entryMap = new Map(entries.map((entry) => [entry.id, entry]));
    const outcomes: SearchLearningOpsPlaybookRecommendationOutcome[] = [];

    for (const run of runs) {
        const relatedEntries = run.entryIds
            .map((entryId) => entryMap.get(entryId))
            .filter((entry): entry is SearchLearningOpsPlaybookRecommendationOutcomeEntryLike => Boolean(entry));
        ...
        let status: SearchLearningOpsPlaybookRecommendationOutcomeStatus = 'validated';
        let description = 'recommendation 실행이 승인 상태와 검색 개선 지표까지 안정적으로 이어졌습니다.';

        if (readyReviewCount > 0) {
            status = 'ready_review';
            description = 'recommendation 실행으로 새 AI suggestion이 준비되어 draft review/승인이 필요한 상태입니다.';
        } else if (noImprovementCount > 0) {
            status = 'needs_attention';
            description = 'recommendation 실행 후에도 low-fit/0건 개선이 약해 재학습 또는 rewrite 조정이 필요합니다.';
        } else if (awaitingSamplesCount > 0 || improvedCount === 0) {
            status = 'awaiting_samples';
            description = 'recommendation은 실행됐지만 아직 표본이 부족해 실제 개선 여부를 더 지켜봐야 합니다.';
        }

        outcomes.push({
            id: `playbook_recommendation_outcome:${run.id}`,
