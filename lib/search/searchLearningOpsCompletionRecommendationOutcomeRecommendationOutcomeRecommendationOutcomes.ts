...
import type { SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun } from './searchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationActivity.ts';

export type SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeStatus =
    | 'ready_review'
    | 'needs_attention'
    | 'awaiting_samples'
    | 'validated';

export type SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeEntryLike = {
    id: string;
    query: string;
...
export type SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome = {
    id: string;
    outcomeId: string;
    title: string;
    action: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun['action'];
    context: string;
    createdAt: string;
    queries: string[];
    entryIds: string[];
    status: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeStatus;
    description: string;
...
export type SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeSummary = {
    total: number;
    readyReview: number;
    needsAttention: number;
    awaitingSamples: number;
    validated: number;
    topReadyReview: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[];
    topNeedsAttention: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[];
    topAwaitingSamples: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[];
    topValidated: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[];
};
...
function sortOutcomes(
    items: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[]
...
export function buildSearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomes(
    runs: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationRun[],
    entries: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeEntryLike[]
): SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeSummary {
    const entryMap = new Map(entries.map((entry) => [entry.id, entry]));
    const outcomes: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcome[] = [];
    ...
    for (const run of runs) {
        const relatedEntries = run.entryIds
            ...
            .filter(
                (
                    entry
                ): entry is SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeEntryLike =>
                    Boolean(entry)
            );
        ...
        let status: SearchLearningOpsCompletionRecommendationOutcomeRecommendationOutcomeRecommendationOutcomeStatus =
            'validated';
        let description =
            'completion recommendation outcome recommendation outcome recommendation 실행이 승인 상태와 검색 개선 지표까지 안정적으로 이어졌습니다.';

        if (readyReviewCount > 0) {
            status = 'ready_review';
            description =
                'completion recommendation outcome recommendation outcome recommendation 실행으로 새 AI suggestion이 준비되어 draft review/승인이 필요한 상태입니다.';
        } else if (noImprovementCount > 0) {
            status = 'needs_attention';
            description =
                'completion recommendation outcome recommendation outcome recommendation 실행 후에도 개선이 약해 추가 재학습 또는 rewrite 조정이 필요합니다.';
        ...
        outcomes.push({
            id: `completion_recommendation_outcome_recommendation_outcome_recommendation_outcome:${run.id}`,
