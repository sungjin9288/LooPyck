...
| Adapter/domain tests | `npm run test:adapters` | 통과, 526 tests pass (2026-09-03 working tree) | `evidence/cli-logs/test-adapters.log` |
...
| Deployment provenance smoke | `npm run ntl:deployment-provenance -- http://localhost:3100` | local build manifest schema·commit 일치; production artifact는 deployment 후 생성 필요 | `output/playwright/{local,netlify}-deployment-provenance.json` |
...
| Netlify production UAT | `npm run ntl:uat` | 현재 배포본 manifest HTTP 404로 첫 provenance step에서 blocking; 후속 4 smoke는 새 build 배포 후 실행 | `output/playwright/netlify-uat-summary.json`, `output/playwright/netlify-deployment-provenance.json` |
| Failure-safe release closeout | `npm run test:release-closeout-contract` + `npm run ntl:release-closeout` | contract 4 case 통과; blocked production에서 UAT/runtime/report exit `[1,0,0]`, 전체 exit 1 | `output/playwright/netlify-release-closeout-execution.json`, `output/playwright/release-closeout-report.md` |
